...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.distributed:
        ...
        train_sampler = None
        val_sampler = None
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True, sampler=train_sampler)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    torch.save(model.module.state_dict(), "temp/params.pth")
    msd = torch.load("temp/params.pth", map_location=torch.device('cpu'))
    # print(msd)
    import pickle as pk
    pk.dump(msd, open("temp/params.pk", "wb"))
    for i, (input, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        loss.backward()
        w_fc = model.module.fc.weight.detach().cpu().numpy()
        w_conv_in = model.module.conv_in.weight.detach().cpu().numpy()
        optimizer.step()
        w_fc_new = model.module.fc.weight.detach().cpu().numpy()
        w_conv_in_new = model.module.conv_in.weight.detach().cpu().numpy()
        data = [input.cpu().numpy(), target.cpu().numpy(), output.detach().cpu().numpy(), loss.detach().cpu().numpy(), w_fc_new - w_fc, w_conv_in_new - w_conv_in]
        import pickle as pk
        pk.dump(data, open("temp/data.pk", "wb"))
        exit(0)
        
        # output2 = model(input)
        # loss2 = smooth_loss(output2, target, args.label_smoothing) if args.label_smoothing else criterion(output2, target)
        # data = [input.cpu().numpy(), target.cpu().numpy(), output.detach().cpu().numpy(), loss.detach().cpu().numpy(), output2.detach().cpu().numpy(), loss2.detach().cpu().numpy()]
        # data
        # import pickle as pk
        # pk.dump(data, open("temp/data.pk", "wb"))
        # exit(0)
...
def tensor2img(tensor):
    mean, std = [0.485, 0.456, 0.406], [0.229, 0.224, 0.225]
    img = tensor.permute([1,2,0]).cpu().detach().numpy() * 256
    return img
